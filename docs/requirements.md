...
**功能描述**：支持创建文件夹、移动、重命名、删除等文件管理操作。文件夹管理采用**前端虚拟文件夹树**设计，基于音频文件的 `storage_path` 路径信息构建，不涉及后端持久化。

**设计原则**：
- 音频文件与项目强绑定，通过 `storage_path` 表达文件夹层级
- 文件夹树形结构由前端根据 `storage_path` 动态构建
- 创建文件夹实际上是创建上传路径，不占用数据库存储
- 文件夹操作仅影响前端展示和后续上传路径
...
1. 用户点击「新建文件夹」按钮
2. 输入文件夹名称
3. 点击「确认」，系统在当前路径下创建虚拟文件夹
4. 新文件夹显示在文件树中，可用于后续上传时选择路径

**移动文件**：
1. 用户选中一个或多个音频文件
2. 点击「移动」按钮或右键选择「移动到」
3. 在弹出的目录树中选择目标位置
4. 点击「确认」，系统更新音频文件的 `storage_path` 字段
5. 文件夹树自动刷新，显示文件移动后的位置

**重命名**：
1. 用户右键点击音频文件
2. 选择「重命名」
3. 输入新名称，按 Enter 确认
4. 系统更新音频文件的 `file_name` 和 `storage_path` 字段
...
**技术实现**：
- 前端根据 `storage_path` 构建虚拟文件夹树
- 路径分隔符：`/`
- 示例路径：`会议记录/2024/Q1/会议001.mp3`
- 支持多级嵌套文件夹（无限层级）
